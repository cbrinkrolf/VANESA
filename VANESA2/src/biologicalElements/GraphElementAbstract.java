...
public class GraphElementAbstract {
	...
	/** checks if the given BiologicalNodeAbstract is equal to this one 
	 *  nodes are equal if name OR label match (also when name matches the label of the other node)
	 */
	/*public boolean equals(Object o) {
		
		if (!(o instanceof BiologicalNodeAbstract)) {
		...
		String name2 = bna.getName();
		String label2 = bna.getLabel();
	...
	public boolean containedInAllOriginalGraphs(Pathway[] pathways) {
		...
		for(int i=1; i<pathways.length; i++) contained = contained && this.getOriginalGraphs().contains(i);
		return contained;
	}
