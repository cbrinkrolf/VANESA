...
public class GraphTheoryAlgorithms extends Object {
	...
	public static void allShortestPathLength(int n, int m, int nodei[],
			int nodej[], boolean directed[], int weight[], int root,
			int mindistance[]) {
					...
					if (nodev >= 0) {
						...
						if (ix == 0) {
							...
							do {
								if (nodey != 0) {
									index++;
									cpoly3[nodeu] = nodei[nodey] + nodej[nodey]
											- nodeu;
									nodei[nodey] = -index;
									nodej[nodey] = nodeu;
								}
	...
	public static void connectedComponents(int n, int m, int nodei[],
	...
	public static void EulerCircuit(int n, int m, boolean directed,
		...
		while (true) {
			if (traillength == 1) {
				...
				p = traillength - 1;
				if (traillength != 2)
					endnode[p] = nodei[trail[p]] + nodej[trail[p]]
							- endnode[p - 1];
				k = endnode[p];
				if (directed)
					for (i = 1; i <= m; i++)
	...
	public static void fundamentalCycles(int n, int m, int nodei[],
			int nodej[], int fundcycle[][]) {
	...
	public static int graphIsomorphism(int n, int adj1[][], int adj2[][],
		...
		for (i = 1; i <= n; i++)
			for (j = 1; j <= n; j++)
				edges1 = (i == j) ? edges1 + 2 * adj1[i][j] : edges1
						+ adj1[i][j];
		edges1 /= 2;
		edges2 = 0;
		for (i = 1; i <= n; i++)
			for (j = 1; j <= n; j++)
				edges2 = (i == j) ? edges2 + 2 * adj2[i][j] : edges2
						+ adj2[i][j];
		edges2 /= 2;
		if (edges1 != edges2)
			return 1;
		...
		k = 1;
		while (k <= n) {
			if (degree1[k] < degree2[k])
				return 2;
			k++;
		}
		// compute the code of the first graph
		if (!isomorphicCode(adj1, n, label1, map1))
			return 4;
		// compute the code of the second graph
		if (!isomorphicCode(adj2, n, label2, map2))
			return 4;
	...
	public static void heapsort(int n, int x[], boolean ascending) {
		/*
		 * sort array elements x[1], x[2],..., x[n] in the order of increasing
		 * (ascending=true) or decreasing (ascending=false)
		 */
	...
	public static void kShortestPaths(int n, int m, int nodei[], int nodej[],
			int weight[], int k, int source, int sink, int pathnodes[],
			int dist[][], int path[][]) {
	...
	public static void kShortestPathsNoRepeatedNodes(int n, int m, int nodei[],
			int nodej[], int weight[], int k, int source, int sink,
			int path[][]) {
					...
					if (!force) {
						...
						if (jump == 1) {
							...
							while (ncrs1 != ncrs3) {
								...
								lengb -= weight[j];
								ncrs1 = (shortpathtree[ncrs1] > 0) ? nodei[j]
										: nodej[j];
							}
							if (!finem1) {
								jump = 1;
					...
					do {
						...
						do {
							if (!force) {
								...
								// obtain the restricted shortest path from
								// source to jterm
								lenrst = upbound;
								bedge = 0;
								auxdist[jterm] = 0;
								...
								if ((bedge != 0) && (lenrst < upbound)) {
									...
									cedge = bedge;
									do {
										dedge = (cedge > 0) ? nodej[cedge]
												: nodei[-cedge];
										if ((cedge != shortpathtree[dedge])
												|| (dedge == jterm)) {
											detb++;
											auxstore[detb] = cedge;
										}
									...
									while (rdfull || (deta > 0)) {
										...
										noroom = true;
										rdfull = false;
										// get the last entry from the priority
										// queue
										if (nqsize > 0) {
									...
									if (!skip) {
										// build the entries of crossarc and
										// nextpoolentry
										if (lenrst >= quelast) {
											if (nostg) {
												resultfound = true;
	...
	public static void maximumConnectivity(int n, int k, int nodei[],
			int nodej[]) {
	...
	public static void minSumMatching(int n, double weight[][], int sol[]) {
		...
		iterate: while (true) {
			if (fin) {
				// generate the original graph by expanding all shrunken
				// blossoms
				skip = false;
				value = 0.;
				for (i = 1; i <= n; i++)
	...
	public static void minimumSpanningTreeKruskal(int n, int m, int nodei[],
		...
		while (i > 0) {
			...
			while (index1 <= halfm) {
				index = index1 + index1;
				index2 = ((index < m) && (weight[index + 1] < weight[index])) ? index + 1
						: index;
		...
		while ((treearc < n - 1) && (numarc < m)) {
			...
			while (index1 <= nedge2) {
				index = index1 + index1;
				index2 = ((index < nedge) && (weight[index + 1] < weight[index])) ? index + 1
						: index;
				if (weight[index2] < weight[index1]) {
					nodeu = nodei[index1];
					nodev = nodej[index1];
	...
	public static void minimumSpanningTreePrim(int n, int dist[][], int tree[]) {
		...
		for (i = 1; i <= n1; i++) {
			...
			for (j = 1; j <= n1; j++) {
				node = tree[j];
				if (node <= 0)
					if (dist[j][k] < dist[j][-node])
						tree[j] = -k;
			}
		}
	}

	public static void multipleKnapsack(int n, int m, int profit[],
			int weight[], int capacity[], int depth, int sol[]) {
		...
		mkpsCurrentUpperBound(n, m, profit, weight, capacity, 1, netp, res1,
		...
		while (true) {
			...
			mkpsFeasibleSolution(n, m, profit, weight, idx, netp, res2, aux1,
					aux2, aux3, aux7, aux8, aux9, aux10, aux11, aux12, aux14,
					aux15);
			...
			if (!outer) {
				...
				while (true) {
					if (aux3[idx] != 0) {
						...
						for (y = 1; y <= tmp; y++) {
							if (aux14[idx][y] != 0) {
								...
								mkpsCalculateBound(idx, idx, res3, control,
										slackbnd, upperbnd, indexj, indexi, n,
										res1, res1a, ubtmp, aux0, aux2, aux5,
										aux6, aux9, aux12);
								if (!control[0]) {
									netp = totalprofit[0] - slackbnd;
									// compute an upper bound of the current
									// solution
									mkpsCurrentUpperBound(n, m, profit, weight,
											aux2, idx, netp, res1, res3, aux5,
											aux7, aux8, aux9, aux10, aux11);
									indexj = n1;
								}
								ubslack = res3[0] + slackbnd;
					...
					idx2 = idx + 1;
					// upper bound computation
					mkpsCalculateBound(idx2, idx, res3, control, slackbnd,
							upperbnd, indexj, indexi, n, res1, res1a, ubtmp,
							aux0, aux2, aux5, aux6, aux9, aux12);
					if (!control[0]) {
						netp = totalprofit[0] - slackbnd;
						// compute an upper bound of the current solution
						mkpsCurrentUpperBound(n, m, profit, weight, aux2, idx2,
								netp, res1, res3, aux5, aux7, aux8, aux9,
								aux10, aux11);
						indexj = n1;
					}
	...
	public static void nodeColoring(int n, int m, int nodei[], int nodej[],
			int color[]) {
		...
		do {
			if (more) {
				...
				if (index > paint + 1)
					index = paint + 1;
				while ((availc[currentnode][newc] < currentnode)
						&& (newc <= index))
					newc++;
	...
	public static int minCostNetworkFlow(int nodes, int edges, int numdemand,
		...
		valuez = 0;
		for (p = 1; p <= edges; p++) {
			if ((nodei[p] > nodes) || (nodej[p] > nodes)
					|| (upbound[p] >= maxint))
				return 3;
			if (upbound[p] == 0)
				upbound[p] = maxint;
	...
	public static int oneShortestPath(int n, int m, int nodei[], int nodej[],
		...
		do {
			if (minlength1 <= minlength2) {
				...
				level1 = 0;
				for (i = 1; i <= n; i++) {
					// finds minlength2 and stores in stack2[1:level2] all
					// values
					// of k such that tosink[k] = minlength2 and tosink[i] =
					// key2
					temp = fromsource[i];
					if (temp > key1) {
						if (temp < minlength1) {
	...
	public static boolean planarityTesting(int n, int m, int nodei[],
			int nodej[]) {
		...
		middle[0] = false;
		do {
			planarityDFS1(n, m, m2, nm2, level, middle, snum, store1, mark,
					firstlow, secondlow, wkpathfind5, wkpathfind6, stackarc,
					first, second);
		} while (level[0] > 1);
		for (i = 1; i <= n; i++)
		...
		middle[0] = false;
		do {
			planarityDecompose(n, m, n2, m22, m33, nm2, nmp2, m7n5, level,
					middle, initp, snode, pnum, nexte, store2, store3, store4,
					store5, trail, descendant, nodebegin, wkpathfind5, start,
					finish, first, second, wkpathfind1, wkpathfind2,
					wkpathfind3, wkpathfind4, nextarc, arctop, arctype);
		} while (level[0] > 1);
		...
		while (tnum <= pnum[0]) {
			...
			middle[0] = false;
			do {
				planarityTwoColoring(m, nmp2, m7n5, level, middle, fail,
						wkpathfind5, paint, nextarc, arctop, examin, arctype);
				if (fail[0])
					return false;
			} while (level[0] > 1);
			while (!examin[tnum])
	...
	public static void quadraticAssignment(int n, int a[][], int b[][],
			int sol[]) {
		...
		qapsubprog2(n, k, npp, subcripta, subcriptb, a, b, aux4, aux7, work1,
				work2, aux16, aux17, aux13);
			...
			obtainbound: while (true) {
					...
					if (partialobj + partialz >= bestobjval) {
						...
						if (!contr) {
							...
							if (aux6[k + 1] + partialobj >= bestobjval) {
								...
								qapsubprog3(n, k, npp, aux15, aux4, aux5,
										contr, aux16, aux17, aux18);
								continue iterate;
							}
						}
						skip = true;
				...
				if (skip) {
					...
					if (!contr) {
						...
						qapsubprog3(n, k, npp, aux15, aux4, aux5, contr, aux16,
								aux17, aux18);
						continue iterate;
					}
					...
					if (k == (n - 2)) {
						...
						while (true) {
							partialz = aux19[valx][j] + aux19[vale][valp]
									+ a[valx][vale] * b[j][valp]
									+ a[vale][valx] * b[valp][j];
							work1[valx] = false;
							work2[j] = false;
				...
				if (valf < 1) {
					...
					if (valf == 1) {
						...
						for (i = 1; i <= npp; i++) {
							valg = aux15[valc];
							valc += npp;
							if ((valg < leastw) && (subcriptd != aux12[i]))
								leastw = valg;
						}
						leasty = leastw;
						i = 1;
						while (i <= npp) {
							if (aux12[i] == subcriptd)
								break;
							i++;
						}
						vale = i;
				...
				qapsubprog3(n, k, npp, aux15, aux4, aux5, contr, aux16, aux17,
						aux18);
				valx = 0;
				for (i = 1; i <= n; i++)
					if (!work1[i]) {
	...
	public static void randomBipartiteGraph(int n1, int n2, int m, long seed,
			int nodei[], int nodej[]) {
		...
		if (m != 0) {
			...
			for (nodea = 1; nodea <= n1; nodea++)
				for (nodeb = n1 + 1; nodeb <= n; nodeb++)
					adj[nodea][nodeb] = adj[nodeb][nodea] = (ran.nextInt(2) == 0) ? false
							: true;
		}
	...
	public static int randomConnectedGraph(int n, int m, long seed,
			boolean weighted, int minweight, int maxweight, int nodei[],
		...
		for (nodea = 2; nodea <= n; nodea++) {
			...
			if (weighted)
				weight[numedges] = (int) (minweight + ran.nextDouble()
	...
	public static void randomMaximumFlowNetwork(int n, int m, long seed,
			int minweight, int maxweight, int nodei[], int nodej[],
			int weight[]) {
	...
	static private boolean isomorphicCode(int adj[][], int n, int label[][],
			int map[]) {
	...
	static private void msmSubprograma(int kk, int n, double big,
			double cost[], int aux1[], int aux2[], int aux3[], int aux4[],
			int aux5[], int aux6[], int aux8[], double work1[], double work2[],
	...
	static private void msmSubprogramb(int kk, int n, double big,
			double cost[], int aux1[], int aux2[], int aux3[], int aux4[],
			int aux5[], int aux7[], int aux9[], double work1[], double work2[],
			double work3[], double work4[]) {
		/* this method is used internally by minSumMatching */
	...
	static private void mkpsCurrentUpperBound(int n, int m, int profit[],
			int weight[], int aux2[], int i, int netp, int res1[], int res3[],
			int aux5[], int aux7[], int aux8[], int aux9[], int aux10[],
			int aux11[]) {
	...
	static private void mkpsCalculateBound(int i, int p, int res3[],
			...
			int indexi, int n, int res1[], int res1a, int ubtmp, int aux0[],
			int aux2[], int aux5[], int aux6[], int aux9[], int aux12[][]) {
		/* this method is used internally by multipleKnapsack */
	...
	static private void mkpsSingleKnapsack(int n, int ns, int ref1[], int netp,
			int ref2[], int aux8[], int aux10[], int aux11[]) {
		/* this method is used internally by multipleKnapsack */
		...
		while (true) {
					...
					if (in <= ns) {
						for (j = in; j <= ns; j++) {
							...
							if (aux11[j] > index2) {
								...
								if (index2 == 0)
									break;
								if (ref2[0] >= index5 + index3 + index2
										* aux10[j] / aux11[j]) {
									middle = true;
									break;
								}
			...
			while (true) {
				...
				over = false;
				while (true) {
					if (ref2[0] >= index5 + ref1[0] * aux10[index6]
							/ aux11[index6]) {
						over = true;
						break;
					}
				...
				if (!over) {
					n = index6 + 1;
					if (ref2[0] < index5 + aux10[index6] + t * aux10[n]
							/ aux11[n])
						break;
				} else
					over = false;
	...
	static private void planarityDFS1(int n, int m, int m2, int nm2,
			int level[], boolean middle[], int snum[], int store1[],
			int mark[], int firstlow[], int secondlow[], int wkpathfind5[],
			int wkpathfind6[], int stackarc[], int first[], int second[]) {
	...
	static private void planarityDFS2(int n, int m, int m2, int nm2,
			int level[], boolean middle[], int snum[], int store1[],
			int mark[], int wkpathfind5[], int stackarc[], int first[],
			int second[]) {
		/* this method is used internally by planarityTesting */
	...
	static private void planarityDecompose(int n, int m, int n2, int m22,
			int m33, int nm2, int nmp2, int m7n5, int level[],
			boolean middle[], int initp[], int snode[], int pnum[],
			int nexte[], int store2[], int store3[], int store4[],
			int store5[], int trail[], int descendant[], int nodebegin[],
			int wkpathfind5[], int start[], int finish[], int first[],
			int second[], int wkpathfind1[], int wkpathfind2[],
			int wkpathfind3[], int wkpathfind4[], int nextarc[], int arctop[],
			boolean arctype[]) {
		/* this method is used internally by planarityTesting */
	...
	static private void planarityTwoColoring(int m, int nmp2, int m7n5,
			int level[], boolean middle[], boolean fail[], int wkpathfind5[],
			int paint[], int nextarc[], int arctop[], boolean examin[],
			boolean arctype[]) {
		/* this method is used internally by planarityTesting */
	...
	static private void qapsubprog1(int n, int aux15[], int aux12[], int large,
	...
	static private void qapsubprog2(int n, int k, int npp, int subcripta,
			int subcriptb, int a[][], int b[][], int aux4[], int vekt[],
			boolean work1[], boolean work2[], int aux16[], int aux17[],
			int aux13[]) {
		/* this method is used internally by quadraticAssignment */
	...
	static private void qapsubprog3(int n, int k, int npp, int aux15[],
			int aux4[], int aux5[], boolean contr, int aux16[], int aux17[],
			int aux18[]) {
		/* this method is used internally by quadraticAssignment */
	...
	static private void qapsubprog4(int n, int large, int c[], int parm[],
			int wk1[], int wk2[], int wk3[], int wk4[], int wk5[], int wk6[],
			int wk7[], boolean wk8[]) {
		/* this method is used internally by quadraticAssignment */
	...
	static private void tspsearch(int nodes, int edges, int weight,
			int dist[][], int row[], int column[], int cursol[], int front[],
			int back[]) {
		...
		if (weight < dist[0][0]) {
			if (edges == (nodes - 2)) {
				...
				for (i = candc; i <= elms - 1; i++)
					colvec[i] = column[i + 1];
				tspsearch(nodes, edges + 1, weight, dist, rowvec, colvec,
						cursol, front, back);
				...
				if (thresh < dist[0][0]) {
					...
					tspsearch(nodes, edges, weight, dist, row, column, cursol,
							front, back);
