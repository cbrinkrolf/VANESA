 ...
 * version supports SBML Level 2 Version 3 only! Please read the documentation
 * while changing the internal methods of this class.
 * 
 * @author mwesterm
 * 
 */
public class SBMLoutputNoWS {
	...
	/**
	 * This constructor needs a file to which the output can be written and a
	 * Pathway to get all information out of the visualized graph.
	 * 
	 * @param file
	 *            File, the new SBML file.
	 * @param pathway
	 *            Pathway, the graph information are received from here.
	 */
	public SBMLoutputNoWS(File file, Pathway pathway) {
		this.file = file;
	...
	/**
	 * Generates a SBML document via a web service. This may take a few seconds.
	 * 
	...
	private void setUpPayload() {
		...
		// create payload
		this.payload = this.factory.createOMElement(new QName("PAYLOAD"));
		...
		this.sbmlTag.addAttribute(this.factory.createOMAttribute(
				XMLConstraints.SBML_ID_LEVEL.getXMLTag(), null,
				XMLConstraints.SBML_LEVEL_VALUE.getXMLTag()));

		// version attribute for sbml
		this.sbmlTag.addAttribute(this.factory.createOMAttribute(
				XMLConstraints.SBML_ID_VERSION.getXMLTag(), null,
				XMLConstraints.SBML_VERSION_VALUE.getXMLTag()));
		...
		this.model.addAttribute(this.factory.createOMAttribute(
				XMLConstraints.MODEL_ID.getXMLTag(), null,
				XMLConstraints.MODEL_ID_VALUE.getXMLTag()));
		// System.out.println(this.payload.toString());
		// set up some additional configurations
		this.payload.setLocalName(XMLConstraints.TRANSFROM_OBJECT_TO_SBML
				.getXMLTag());
		this.payload = this.sbmlTag;
		// System.out.println(this.payload.toString());
		// System.out.println(this.payload.getChildrenWithName());
	...
	private OMElement sendPayLoad() {
		...
		try {
			...
			System.out
					.println("Payload: " + this.payload.toStringWithConsume());

			// send the sbml data to web service
			result = null;// sender.sendReceive(this.payload);
	...
	/**
	 * writeSBMLDocument The SBML file will be written in this method. Further
	 * file handling like already existing files etc. is <b>still not
	 * implemented</b>.
	 * 
	 * @param sbmlDoc
	 *            OMElement, which contains the SBML file data.
	 */
	private String writeSBMLDocument(OMElement sbmlDoc) {
	...
	/**
	 * If compartment type information are available, they will be returned in a
	 * web service understandable manner (OMElement).
	 * 
	 * @param oneNode
	 *            one Node in the application�s graph.
	 * 
	 *            Adds a OMElement, a XML tag with given exactly to the payload
	 *            (which will be send as a package to the web service). The
	 *            OMElement contains one attribute: "id". This attribute
	 *            represents a relationship between multiple compartments. @see
	 *            getListOfCompartmentTypes().
	 */
	private void createListOfCompartmentTypes(BiologicalNodeAbstract oneNode) {
		if (oneNode != null) {
			String compType = oneNode.getSbml().getCompartment();
			if (compType != null) {
				...
				if (!this.availableCompartmentTypes.keySet().contains(compType)) {
					...
					String compID = XMLConstraints.COMPARTMENT_TYPE_NAME
							.getXMLTag() + compCount;
					...
					OMElement compTypeOm = this.factory.createOMElement(
							new QName(XMLConstraints.COMPARTMENT_TYPE
									.getXMLTag()), this.listOfCompartmentTypes);

					// add id attribute
					compTypeOm.addAttribute(this.createOMAttribute(
							XMLConstraints.ID.getXMLTag(), compID));

					// add name attribute
					compTypeOm.addAttribute(this.createOMAttribute(
							XMLConstraints.NAME.getXMLTag(), compType));
	...
	private void createListOfSpeciesTypes(BiologicalNodeAbstract oneNode) {

		if (oneNode != null) {
			String speciesType = oneNode.getSbml()
					.getBiologicalNodeDescription();
			if (speciesType != null
					&& (!this.availableSpeciesTypes.keySet().contains(
							speciesType))) {
				int speciesCount = (this.availableSpeciesTypes.size() + 1);
				String id = XMLConstraints.SPECIES_ID.getXMLTag()
						+ speciesCount;
				this.availableSpeciesTypes.put(speciesType, id);

				// create species type list
				OMElement species = this.factory.createOMElement(new QName(
						XMLConstraints.SPECIES_TYPE.getXMLTag()),
						this.listOfSpeciesType);

				// add id attribute
				species.addAttribute(this.createOMAttribute(
						XMLConstraints.ID.getXMLTag(), id));

				// add name attribute
				species.addAttribute(this.createOMAttribute(
						XMLConstraints.NAME.getXMLTag(), speciesType));
			}
		}
	}
	 ...
	 *            one Node in the application�s graph.
	 * 
	 *            Adds an OMElement, a XML tag with given exactly to the payload
	 *            (which will be send as a package to the web service). The
	 *            OMElement contains two attributes: "id" and "compartmentType".
	 */
	private void createListOfCompartments(BiologicalNodeAbstract oneNode) {

		if (oneNode != null) {
			// the compartment type id
			String compType = this.availableCompartmentTypes.get(oneNode
					.getSbml().getCompartment());

			String vertex = oneNode.getSbml().getVertex();
			if (compType != null && vertex != null
					&& (!this.availableCompartments.keySet().contains(vertex))) {
				int compCount = (this.availableCompartments.size() + 1);
				String id = XMLConstraints.COMARTMENT_NAME.getXMLTag()
						+ compCount;
				this.availableCompartments.put(vertex, id);

				// generate compartment and add it to compartment list
				OMElement compartment = this.factory.createOMElement(new QName(
				...
				compartment.addAttribute(this.createOMAttribute(
						XMLConstraints.COMPARTMENT_TYPE.getXMLTag(), compType));
	 ...
	 *            Adds a OMElement, a XML tag with given exactly to the payload
	 *            (which will be send as a package to the web service). The
	 *            OMElement contains one attribute: "id".
	 */
		...
		if (oneNode != null) {
			...
			if (vertexID != null) {
				...
				String speciesType = this.availableSpeciesTypes.get(oneNode
						.getSbml().getBiologicalNodeDescription());
				...
				if (compartment != null && speciesType != null && name != null) {
					...
					// attribute compartment tag
					species.addAttribute(this.createOMAttribute(
							XMLConstraints.COMPARTMENT.getXMLTag(), compartment));

					// attribute id
					species.addAttribute(this.createOMAttribute(
							XMLConstraints.ID.getXMLTag(), vertexID));
					...
					// attribute name
					species.addAttribute(this.createOMAttribute(
							XMLConstraints.NAME.getXMLTag(), name));
				}
			}
		}
	}
	...
	/**
	 * Creates service understandable parameter type. bna
	 * 
	 * @param oneEdge
	 *            one reaction in the applications graph
	 * 
	 *            Adds a OMElement.
	 */
	private void createListOfParameter(BiologicalNodeAbstract oneNode) {
		if (oneNode != null) {
			...
			if (vertexID != null) {
				...
				OMElement xParameter = this.factory.createOMElement(new QName(
						XMLConstraints.PARAMETER.getXMLTag()),
						this.listOfParameter);
				// set id
				String xId = XMLConstraints.PARAMETER_SID.getXMLTag() + ""
						+ this.parameterCount;
				xParameter.addAttribute(this.createOMAttribute(
						XMLConstraints.PARAMETER_ID.getXMLTag(), xId));
				// set name
				xParameter.addAttribute(this.createOMAttribute(
						XMLConstraints.PARAMETER_NAME.getXMLTag(), name));
				// set x-coordinate
				xParameter
						.addAttribute(this.createOMAttribute(
				...
				// create Y-Parameter
				OMElement yParameter = this.factory.createOMElement(new QName(
						XMLConstraints.PARAMETER.getXMLTag()),
						this.listOfParameter);
				// set id
				String yId = XMLConstraints.PARAMETER_SID.getXMLTag() + ""
						+ this.parameterCount;
				yParameter.addAttribute(this.createOMAttribute(
						XMLConstraints.PARAMETER_ID.getXMLTag(), yId));
				// set name
				yParameter.addAttribute(this.createOMAttribute(
						XMLConstraints.PARAMETER_NAME.getXMLTag(), name));
				// set y-coordinate
		...
		if (oneEdge != null) {
			...
			if (from != null && to != null && name != null
				...
				String id = XMLConstraints.REACTION_NAME.getXMLTag()
						+ (this.availableReactions.size() + 1);
				this.availableReactions.put(name, id);
				// generate species and add it to compartment list
				OMElement reaction = this.factory.createOMElement(new QName(
						XMLConstraints.REACTION.getXMLTag()),
				...
				OMElement listOfProduct = this.factory.createOMElement(
						new QName(XMLConstraints.LIST_OF_PRODUCTS.getXMLTag()),
						reaction);

				// species reference for product
				OMElement speciesReference2 = this.factory
						.createOMElement(new QName(
								XMLConstraints.SPECIES_REFERENCE.getXMLTag()),
								listOfProduct);
	 ...
	 * @return OMAttribute, the attribute. Null if no attribute has been
	 *         generated.
	 */
	private OMAttribute createOMAttribute(String tag, String value) {

		if (tag != null && value != null) {
			OMAttribute attribute = this.factory.createOMAttribute(tag, null,
					value);
			return attribute;
		}
		return null;
