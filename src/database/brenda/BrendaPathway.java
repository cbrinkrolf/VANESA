...
public class BrendaPathway extends SwingWorker {
	...
	private void processBrendaElement(String enzyme, DefaultMutableTreeNode node) {
		...
		if (node.getLevel() == 0 || (node.getLevel() / 2) < searchDepth) {
			...
			String[] resultDetails;
			String clean;
			System.out.println(enzyme+": "+results.size());
			for (DBColumn column : results) {
				...
				if (!enzymes.containsKey(clean)) {
					...
					enzymes.get(clean).setColor(Color.RED);
				}
			}
		}
	}
	...
	private void getEnzymeDetails(String[] details) {
		...
		Iterator<String> it = this.enzymes2Search.iterator();
		 while(it.hasNext()){
			 processBrendaElement(it.next(), tree.getRoot());
		 }
	...
	public void done() {
		...
		if (answer == JOptionPane.YES_OPTION || answer == JOptionPane.NO_OPTION) {
			...
			//System.out.println(enzymeToSearch);
