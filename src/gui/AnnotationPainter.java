...
public class AnnotationPainter {
	...
	public void setCurrentRangeType(int currentRangeType) {
		...
		RenderContext<BiologicalNodeAbstract, BiologicalEdgeAbstract> rc = vv
				.getRenderContext();
		annotatingPlugin = new MyAnnotatingGraphMousePlugin<BiologicalNodeAbstract, BiologicalEdgeAbstract>(
				rc);
		annotatingPlugin.setCursor(Cursor
		...
		graphMouse.getModeComboBox().setSelectedItem(
	...
	private Color getColor(Color oldColor) {
		...
		try {
			newColor = JColorChooser.showDialog(MainWindow.getInstance().getFrame(), "select a new color.",
					oldColor);
		} catch (Exception e) {
