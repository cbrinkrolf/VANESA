...
public class DataMappingModelController extends Observable {
	...
	private void doCheck(int isSet) {
		...
		// network is only need for colaration/ species is only needed for mapping into
		// db
	...
	/**
	 * starts the BioMart query, sets the query results in the datamappingModel and
	 * starts the main work of the dataMappingModel
	 */
	public void startMapping() {
		...
		if (detectPathwayOrigin().equals("HPRD")) {
			...
			for (int i = 0; i < numOfThreads; i++) {
				if (i < labelsPartMod) {
					tasksQuery.add(new QueryParallel(new HPRDQueryRetrieval("homo sapiens"),
							labels.subList(labelsCounter, (labelsCounter + labelsPartLength + 1)), identifierType));
					labelsCounter = labelsCounter + labelsPartLength + 1;

				} else { // letzter Block
					tasksQuery.add(new QueryParallel(new HPRDQueryRetrieval("homo sapiens"),
							labels.subList(labelsCounter, (labelsCounter + labelsPartLength)), identifierType));
			...
			for (int i = 0; i < numOfThreads; i++) {
				if (i < labelsPartMod) {
					tasksQuery.add(new QueryParallel(new IntActQueryRetrieval(detectPathwaySpeciesAll()),
							labels.subList(labelsCounter, (labelsCounter + labelsPartLength + 1)), identifierType));
					labelsCounter = labelsCounter + labelsPartLength + 1;
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "Could not execute query.", "Error", JOptionPane.ERROR_MESSAGE);
