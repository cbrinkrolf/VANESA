...
public class DataMappingView extends JDialog implements Observer {
	...
	private String[] identifier = { "none", "Agilent [e.g. A_23_P30024] ", "Affymetrix [e.g. 209239_at]",
			"EMBL [e.g. M58603]", "UniProt [e.g. P19838]", "Illumina [e.g. ILMN_1674519]" };
	...
	public DataMappingView() {
		...
		panelSource.add(new JLabel("Second Step - Select or create (by right click on the table) a network."),
				"wrap, span, growx");
		...
		panelData.add(new JLabel("<html>Third Step - <br></br>Select the row with your header labels</html>"),
				"span, wrap");
		panelData.add(headerTF = new JTextField("Click in one cell with a header label"), "span, growx, wrap 15");
		headerTF.setEditable(false);
		panelData.add(new JLabel("<html>Fourth Step - <br></br>Select the type of your identifiers</html>"),
				"span, wrap");
		panelData.add(new JLabel("Type of identifier:"), "wrap");
		identifierCB = new JComboBox<>(identifier);
		identifierCB.setActionCommand("identifier");
		panelData.add(identifierCB, "span, growx, wrap 15");
		panelData.add(new JLabel("<html>Fifth Step - <br></br>Select the column with identifiers</html>"),
				"span, wrap");
		panelData.add(identifierTF = new JTextField(clickHeader), "span, growx, wrap 15");
		identifierTF.setEditable(false);
		panelData.add(new JLabel("<html>Sixth Step - <br></br>Select the column with data values</html>"),
				"span, wrap");// to be mapped to the network</html>"), "span, wrap");
	...
	/**
	 * checks whether there is a new pathway in the MainWindow, if after a new search there is one new pathway than it
	 * returns true. Do not call if you have not executed a new search!
	 * 
	...
	public void initPathwayComboBox() {
		...
		if (pathwayCB == null) {
			...
			for (Pathway pw : con.getAllPathways()) {
				pathwayList.add(pw.getName());
			}
			pathwayCB = new JComboBox<>(pathwayList.toArray(new String[0]));
			pathwayCB.addItem("none");
		} else {
			pathwayCB.removeAllItems();
			pathwayCB.addItem("none");
			for (Pathway pw : con.getAllPathways()) {
				pathwayCB.addItem(pw.getName());
			}
		}
	...
	public void update(Observable o, Object arg) {
		...
		if (arg.getClass().equals(Boolean.class)) {
			okButton.setEnabled((Boolean) arg);
		}
	...
	/**
	 * creates the dataMapping tableModel with the new data from the model after a search or after some data adjustments
	 * have taken place
	 *
	...
	private void setDataMappingTableModel(DataMappingModel dmm) {
		...
		for (Entry<String, List<String>> entry : mergeMap.entrySet()) {
			Vector<Object> rowDataV = new Vector<>();
			...
			if (!dups.containsKey(entry.getKey())) { // a unique entry
				...
				for (int i = 0; i < numOfDups; i++) {
					Vector<Object> rowDataD = new Vector<>();
	...
	/**
	 * add the Components (ColorGradient, MappingTable and ChangeButton to the dataAdjustment tabbedPane
	 */
	private void addAdjustmentComponents() {
		...
		dataMappingScroll = new JScrollPane(dataMappingTable, ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,
	...
	/**
	 * some messages to be shown on top of the main Frame and the actions after the "OK" button is pressed
	 * 
	...
	/**
	 * closes the ProgressBar after the JTable was updated, sets boolean done (whether the input is finished) to false
	 * after all work is done, after that it is possible to start a new ProgressBar
	 */
	...
	/**
	 * displays the gradient of the color code together with minimal, maximal, and average values of the data in a
	 * slider
	 * 
	...
	private void setGradientSlider(DataMappingModel dmm) {
		...
		String averS = "<html><b><FONT COLOR=#D3D3D3>" + "<br></br>Black from " + lowerB + " to " + upperB
				+ "</FONT></b></html>";
	...
	/**
	 * the boolean inToolSearch variable lets the updating for the pathway ComboBox decide if a new pathway or the
...
/**
 * This class creates a customize slider with a color gradient in the background. This color gradient is based on the
 * values of the data input and the settings are calculated from the sliders min, max and aver values
 * 
 * @author dborck
 *
 */
class MySlider extends JSlider {
	...
	/**
	 * constructs the slider, important is to set Opaque(false) because you want to take care of the color printing
	 * 
	 * @param min
	 * @param max
	 * @param aver
