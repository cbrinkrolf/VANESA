...
public class DoSmacof extends Thread {
	...
	public DoSmacof(HashMap<Integer, double[]> data_map, HashMap<BiologicalNodeAbstract, Integer> mapped_nodes,
			String dis, int maxiter, double epslion, int p, int resultdim, SmacofView view) {
	...
	public void run() {
		...
		smac = new Smacof();
		Mat result_mat = smac.smacofAlgorithm(this.smacof_data_map, this.maxiter, this.epsilon, this.resultdim,
				this.dis, this.p, false);

//	   System.out.println("Result_Mat:");
//	   System.out.println(result_mat);

		// Skalierung der Ergebnisse um (hoffentlich) besser Darstellung zu erhalten
		// es muss noch auf sinnvolle Weise der Skalierungsfaktor automatisch bestimmt
		// werden!
		...
		Map<BiologicalNodeAbstract, Point2D> vertices = con.getPathway(MainWindow.getInstance().getCurrentPathway())
				.getVertices();
		BiologicalNodeAbstract bna;
		Point2D point = null;
		int id;
		Iterator<BiologicalNodeAbstract> it = con.getPathway(MainWindow.getInstance().getCurrentPathway())
