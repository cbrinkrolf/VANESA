		...
		if (settings.getDrawEdges()) {
			if (psV.getPicked().isEmpty()) {
				if (psE.getPicked().isEmpty()) {
					...
					// System.out.println("t: "+bea.getColor().getTransparency());
					// System.out.println("a: " + bea.getColor().getAlpha());
					if (settings.getEdgeOpacity() < 255) {
						return new Color(bea.getColor().getRed(), bea.getColor().getBlue(), bea.getColor().getGreen(),
								settings.getEdgeOpacity());
					} else {
						return bea.getColor();
					}
