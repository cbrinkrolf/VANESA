...
public class DynamicIcon implements Icon {
    ...
    public void paintIcon(Component c, Graphics g, int x, int y) {
        ...
        String tokens = String.valueOf(p.getToken());
        if (p.isDiscrete()) {
            tokens = String.valueOf((int) p.getToken());
            discrete = true;
        }
        if (p.isLogical() && p.getLogicalReference() instanceof Place) {
            tokens = String.valueOf(((Place) p.getLogicalReference()).getToken());
            if (p.getLogicalReference().isDiscrete()) {
                tokens = String.valueOf((int) ((Place) p.getLogicalReference()).getToken());
                discrete = true;
            }
        }
