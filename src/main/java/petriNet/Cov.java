...
public class Cov {
	...
	public Cov() {
		...
		root = new CovNode("label", "name", numberPlaces, targetPathway);
			...
			if (cl.isGreaterEqualCol(col)) {
				...
				if (isBoundaryHold(tmp)) {
					...
					for (CovNode cn : parents) {
						...
						if (!found) {
							final CovNode n = new CovNode("label2", "name2", numberPlaces, targetPathway);
					...
					if (!found) {
						CovNode n = new CovNode("label2", "name2", numberPlaces, targetPathway);
						n.setTokenList(tmp);
