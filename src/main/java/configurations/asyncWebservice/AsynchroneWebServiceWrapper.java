...
/**
 * This class wraps the asynchrone web service invocation to all databases, like
 * BRENDA, KEGG ... This is a singleton and a observable class to get informed
 * in the case of available data.
 * 
...
public class AsynchroneWebServiceWrapper extends AbstractWebServiceObservable {
	...
	public UUID callWebserviceWithQuery(String database, String query) {
		...
		try {
			// -- default : String
			// webserviceName="http://tunicata.techfak.uni-bielefeld.de/axis2/services/DBService";
			// --
			String webserviceName = ConnectionSettings.getInstance().getWebServiceUrl();
			...
			MyPopUp.getInstance().show("Webservice error:",
	...
	private Options createOptions(EndpointReference endpoint, OMElement request, AxisCallback callback)
			throws AxisFault {
		// -- set up the options --
		...
		if (withAddressing) {
			try {
				...
				JOptionPane.showMessageDialog(null, "Engaging addressing module failed!\nContinue without addressing.",
						"Addressing", JOptionPane.ERROR_MESSAGE);
			}
		} else
			this.serviceClient.disengageModule("addressing");
	}
