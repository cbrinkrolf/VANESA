...
public class DirectionLayout implements LayoutManager2 {
	...
	private HashMap<Component, Insets> componentInsets;
	...
	public void layoutContainer(Container parent) {
		...
		int pos = direction == Direction.RIGHT
				? insets.left
				: direction == Direction.DOWN ? insets.top : direction == Direction.LEFT ? insets.right : insets.bottom;
		...
		for (int i = 0; i < components.length; i++) {
			...
			if (isVertical()) {
				...
				components[i].setLocation(
						direction == Direction.RIGHT ? pos : parent.getWidth() - pos - components[i].getWidth(), yPos);
			}
