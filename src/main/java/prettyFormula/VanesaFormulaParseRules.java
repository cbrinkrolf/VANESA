...
/**
 * ANTLR visitor that parses a VanesaFormula parse tree to a subset of LaTeX commands.
 *
...
public class VanesaFormulaParseRules extends VanesaFormulaBaseVisitor<String> {
     ...
     * {@inheritDoc}
     * <p>
     * From a variable node, the variable is returned as text. If it has a subscript part added to it, the subscript
     * part is recursively wrapped in curly braces, so subscript words with length > 1 are possible.
     *
     * @param ctx Variable context
     * @return The text content of this node. Subscript parts are encapsulated in curly braces.
     */
    @Override
     ...
     * {@inheritDoc}
     * <p>
     * From a negative variable node, the variable is returned as text, as determined by
     * {@link #visitVariable(prettyFormula.VanesaFormulaParser.VariableContext) }.
     *
     * @param ctx Negative variable context
     * @return The variable wrapped in "(-" and ")".
     */
    @Override
    public String visitNeg_variable(VanesaFormulaParser.Neg_variableContext ctx) {
     ...
     * {@inheritDoc}
     * <p>
     * The most important node visitor function. It recognizes the different kinds of terms and applies the custom
     * formatting rules to it. This function contains examples for all possible kinds of terms as well as examples for
     * throwing custom errors properly.
     *
     * @param ctx Term node context.
     * @return Returns the formula string of the term node (including all its children) formatted according to the rules
     * specified in this function.
     */
    @Override
    public String visitTerm(VanesaFormulaParser.TermContext ctx) throws DetailedParseCancellationException {
        String ret;
        // OPERATORS
                ...
                case "sqrt":
                    if (ctx.term().size() > 1) {
                        ...
                        throw new DetailedParseCancellationException(
                                "Square root " + "must not have multiple arguments.", ctx.getStart().getLine(),
