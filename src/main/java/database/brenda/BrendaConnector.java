...
public class BrendaConnector {
	...
	private final Map<String, BiologicalNodeAbstract> enzymes = new HashMap<>();
	...
	private void autoCoarseDepth() {
		...
		class HLC implements HierarchyListComparator<Integer> {
			...
			public Integer getValue(BiologicalNodeAbstract n) {
				...
				int bestDistance = rootDistanceMap.get(n) == null ? Integer.MAX_VALUE
						: rootDistanceMap.get(n).intValue();
				for (BiologicalNodeAbstract neighbor : neighbors) {
					if (rootDistanceMap.get(neighbor) != null
							&& (bestNeighbor == n || rootDistanceMap.get(neighbor).intValue() <= bestDistance)) {
	...
	private String[] enzymesInPathway() {
		...
		for (Map.Entry<String, BiologicalNodeAbstract> entry : enzymes.entrySet()) {
			if (entry.getValue() instanceof Enzyme) {
				result.add(entry.getValue().getLabel());
			}
		}
	...
	public void search() {
		...
		if (mergePW != null) {
			answer = JOptionPane.showOptionDialog(MainWindow.getInstance().getFrame(),
					...
					new String[] { "Selected Pathway Only", "Merge Pathways" }, JOptionPane.CANCEL_OPTION);
		}
