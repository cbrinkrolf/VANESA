...
public class BrendaConnector {
	...
	private void drawEdges() {
		for (BiologicalEdgeAbstract bea : edges) {
			if (graph.findEdge(bea.getFrom(), bea.getTo()) == null) {
				pw.addEdge(bea);
			}
		}
	}
	...
	public void search() {
		...
		if (mergePW != null) {
			answer = JOptionPane.showOptionDialog(MainWindow.getInstance().getFrame(),
					"A new tab will be created with the pathway you selected. Shall "
							...
							+ "or contain only the selected pathway?", "", JOptionPane.YES_NO_OPTION,
					JOptionPane.QUESTION_MESSAGE, null, new String[] { "Selected Pathway Only", "Merge Pathways" },
		...
		if (answer == JOptionPane.YES_OPTION || answer == JOptionPane.NO_OPTION) {
			if (answer == JOptionPane.NO_OPTION) {
				...
				pw = CreatePathway.create("EC: " + enzyme.ec);
			}
			...
			drawNodes();
			drawEdges();
			graph.apply(new GEMLayoutOperation());
