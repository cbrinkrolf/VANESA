...
public class MirnaSearch {
	...
	private static void enrichTargetGeneMatures(Pathway pw, boolean hsaOnly) {
		...
		final Collection<BiologicalNodeAbstract> nodes =
				graph.getSelectedNodeCount() == 0 ? graph.getNodes() : graph.getSelectedNodes();
		...
		PopUpDialog.getInstance().show("Target Gene Enrichment",
				String.format("%d nodes and %d edges have been added!", counterNodes, counterEdges));
	}

	private static void addVertexOnCircle(Pathway pw, BiologicalNodeAbstract parent, BiologicalNodeAbstract child) {
		Point2D p = Circle.getPointOnCircle(pw.getGraph2().getNodePosition(parent), 20, 2 * Math.random() * Math.PI);
