...
public class PNMLOutput extends BaseWriter<Pathway> {
	...
	private void generatePNMLDocument(OutputStream outputStream) throws InvalidIDException, VoidRepositoryException {
		...
		for (Place place : this.nodeList) {
			// if ("Discrete Place".equals(place.getBiologicalElement()) || "Continuous
			// Place".equals(place.getBiologicalElement())) {
		...
		for (Transition transition : this.transitionList) {
			// if ("Discrete Transition".equals(transition.getBiologicalElement()) ||
			// "Continuous Transition".equals(transition.getBiologicalElement())) {
			if (!transitionPNML.containsKey("T_" + transition.getID())) {
				...
				t1 = transitionPNML.get(String.valueOf(transition.getID()));
			}
			// }
		}
		...
		// In order to not break the BaseWriter error handling before writing the actual
		// file, we first write
